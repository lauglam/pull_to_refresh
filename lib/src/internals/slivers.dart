      ...
      switch (refreshStyle) {
        case RefreshStyle.Follow:
          geometry = SliverGeometry(
            ...
            layoutExtent:
                Math.min(needPaintExtent, Math.max(layoutExtent - constraints.scrollOffset, 0.0)),
          );
        ...
        case RefreshStyle.UnFollow:
          geometry = SliverGeometry(
            scrollExtent: layoutExtent,
            paintOrigin: Math.min(-overscrolledExtent - constraints.scrollOffset,
...
class RenderSliverLoading extends RenderSliverSingleBoxAdapter {
  ...
  double? computePaintOrigin(double? layoutExtent, bool reverse, bool follow) {
    if (follow) {
      ...
      if (reverse) {
        return Math.max(
                constraints.viewportMainAxisExtent - constraints.precedingScrollExtent, 0.0) +
            layoutExtent!;
      } else {
        return Math.max(
            constraints.viewportMainAxisExtent - constraints.precedingScrollExtent, 0.0);
...
class RenderSliverRefreshBody extends RenderSliverSingleBoxAdapter {
  ...
  void performLayout() {
    ...
    if (childExtent == 1111111) {
      child!.layout(
          constraints.asBoxConstraints(maxExtent: constraints.viewportMainAxisExtent - 0.1),
          parentUsesSize: true);
    }
    switch (constraints.axis) {
      case Axis.horizontal:
