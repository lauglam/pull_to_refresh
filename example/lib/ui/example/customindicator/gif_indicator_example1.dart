...
class GifHeader1State extends RefreshIndicatorState<GifHeader1>
  ...
  void onModeChange(RefreshStatus? mode) {
    ...
    if (mode == RefreshStatus.refreshing) {
      _gifController.repeat(min: 0, max: 29, period: const Duration(milliseconds: 500));
    }
  ...
  Future<void> endRefresh() {
    ...
    return _gifController.animateTo(59, duration: const Duration(milliseconds: 500));
  }
...
class GifFooter1 extends StatefulWidget {
  const GifFooter1({Key? key}) : super(key: key);
...
class GifIndicatorExample1 extends StatefulWidget {
  const GifIndicatorExample1({Key? key}) : super(key: key);
