...
class HideFooterManual extends StatefulWidget {
  const HideFooterManual({Key? key}) : super(key: key);
...
class HideFooterManualState extends State<HideFooterManual> {
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshConfiguration.copyAncestor(
      ...
      child: LayoutBuilder(
        builder: (b, c) {
          double refresherHeight =
              c.biggest.height - 50.0; // 50.0 is Container's height before SmartRefresher
          return Column(
            children: <Widget>[
              Container(
                height: 50.0,
                  ...
                  child: CustomScrollView(
                    slivers: <Widget>[
                      SliverFixedExtentList(
                        delegate: SliverChildBuilderDelegate((c, i) => Text(strs[i]),
