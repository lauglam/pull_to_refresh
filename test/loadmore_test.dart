...
//    expect(_refreshController.footerStatus, LoadStatus.idle);
  ...
  testWidgets("far from bottom,flip to bottom by ballstic also can trigger loading",
      (tester) async {
    final RefreshController _refreshController = RefreshController(initialRefresh: true);
    await tester.pumpWidget(Directionality(
      textDirection: TextDirection.ltr,
  ...
  testWidgets("when autoLoad = false or enableLoadingWhenFailed = true", (tester) async {
    ...
    expect(
        _refreshController.position!.pixels, _refreshController.position!.maxScrollExtent - 30.0);
    expect(_refreshController.footerStatus, LoadStatus.failed);
    await tester.pump();
  ...
  testWidgets("verity footer trigger distance", (tester) async {
    final RefreshController _refreshController = RefreshController();
    await tester.pumpWidget(Directionality(
      textDirection: TextDirection.ltr,
      child: RefreshConfiguration(
